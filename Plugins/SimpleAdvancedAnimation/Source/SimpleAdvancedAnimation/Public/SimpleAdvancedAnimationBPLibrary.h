...
class SIMPLEADVANCEDANIMATION_API USimpleAdvancedAnimationBPLibrary : public UBlueprintFunctionLibrary
{
	...
	static int32 CreateFootIK(ACharacter* InCharacter, const TArray<FName> InBoneNames, float InTraceDistance = 50.f, float InTraceStart = 50.f, float InInterpSpeed = 18.f);

	UFUNCTION(BlueprintCallable, Category = "AdvancedAnimation|FootIK")
	static FFootIKInfo FindFootIKInfo(const int32 InHandle, const FName& InBoneName);

	UFUNCTION(BlueprintCallable, Category = "AdvancedAnimation|FootIK")
	static float FindFootIKOffset(const int32 InHandle, const FName& InBoneName);

	UFUNCTION(BlueprintCallable, Category = "AdvancedAnimation|FootIK")
	static float CalculateFootIKCentroidOffsetZ(const TArray<float> &InOffsets);

	UFUNCTION(BlueprintCallable, Category = "AdvancedAnimation|FootIK")
	static void DestroyAdvancedAnimSystem();
};
